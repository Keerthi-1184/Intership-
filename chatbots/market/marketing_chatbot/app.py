...
def get_data_dir():
    """Get absolute path to marketing_data directory"""
    current_dir = Path(__file__).parent
    data_dir = current_dir / "marketing_data"
...
def initialize_qa_chain():
    try:
        ...
        if not text_files:
            logger.warning("No text files found in marketing_data directory.")
            return None
        ...
        if not documents:
            logger.warning("No documents loaded from marketing_data directory")
            return None
        ...
        try:
            ...
            test_response = llm("Test marketing slogan generation")
        ...
        prompt_template = """
        You are a marketing content expert. Generate creative, concise, and engaging marketing content (e.g., slogans, social media posts, ad copy) based on the user's prompt.
        Use the provided context to ground your response. If the prompt is unrelated to marketing, respond: "I specialize in marketing content generation only."
...
async def query_endpoint(query: Query):
    ...
    if not qa_chain:
        ...
        return {
            "response": "I'm sorry, the marketing knowledge base is not available. Please ensure the marketing_data directory contains text files and Ollama is running with all required models.",
            "sources": []
        }
    try:
...
async def image_analysis_endpoint(file: UploadFile = File(...)):
    try:
        ...
        # Create prompt for marketing analysis
        prompt = f"""
        Analyze this image for marketing purposes. Suggest how it could be used in a marketing campaign (e.g., social media, ads).
...
async def code_generation_endpoint(query: CodeQuery):
    ...
    try:
        ...
        prompt = f"""
        You are a coding assistant for marketing tools. Generate clean, functional code based on the user's prompt.
        Focus on marketing-related tasks (e.g., HTML for email campaigns, CSS for banners).
        If the prompt is unrelated to marketing code, respond: "I specialize in marketing-related code generation only."
