    ...
    if user_input:
        if user_input.strip().lower() == "exit":
            ...
            with st.spinner("🤖 Generating response..."):
                ...
                try:
                    response = requests.post("http://localhost:8000/chat/", json=payload, timeout=600)
...
st.header("📄 Text Summarization from Files")

uploaded_file = st.file_uploader(
    "Upload a file to summarize (PDF, DOCX, TXT, CSV, HTML, JSON, JPG, PNG)",
    type=["pdf", "docx", "txt", "csv", "html", "htm", "json", "jpg", "jpeg", "png"]
)
...
if uploaded_file is not None:
    ...
    if st.button("Generate Summary"):
        with st.spinner("Summarizing... This may take a while for large files"):
            files = {"file": (uploaded_file.name, uploaded_file.getvalue())}
            data = {"model_name": model_choice}
        try:
            response = requests.post("http://localhost:8000/summarize/", files=files, timeout=600)
            if response.status_code == 200:
